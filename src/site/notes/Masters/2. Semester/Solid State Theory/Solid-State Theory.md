...
| $m$ | free electron mass | $9.1094 \times 10^{-31} \mathrm{~kg}$ |
| :--- | :--- | :--- |
...
### 2.1.1 Space groups of crystals
...
## 2.2 Bloch's theorem and Bloch functions
...
This represents an eigenvalue problem in infinite dimensions with eigenvectors $c_{\boldsymbol{G}}(\boldsymbol{k})$ and eigenvalues $\epsilon_{\boldsymbol{k}}$ as band energies. These $\epsilon_{\boldsymbol{k}}$ are corrections to the bare parabolic dispersion, $\epsilon_{\boldsymbol{k}}^{(0)}=$ $\hbar^{2} \boldsymbol{k}^{2} / 2 m$, due to the potential $V(\boldsymbol{r})$. In the absence of the potential $V(\boldsymbol{r})$ these equations suggest that the band energies consist of infinitely many parabolic bands centered around reciprocal lattice points, given by the reciprocal lattice vectors,
$$
\begin{align*}
...
We now construct an approximation of an extended Bloch states by a linear combination of atomic orbitals (LCAO)
$$
\begin{align*}
...
Note that $t$ and $t^{\prime}$ shrink quickly, if with growing lattice constant $a$ the overlap of atomic orbitals decreases.
...
An alternative approach to the tight-binding approximation is through Wannier functions. These are defined as the Fourier transformation of the Bloch wave functions,
$$
\begin{align*}
...
This is the same type of tight-binding band structure as we have derived above from the LCAO view point.
...
Figure 1.8: Material classes according to band filling: left panel: insulator or semiconductor (partially filled bands with the Fermi level in band gap); center panel: metal (Fermi level inside band); right panel: metal or semimetal (Fermi level inside two overlapping bands).
...
## 2.7 Dynamics of band electrons - semiclassical approach
...
## 2.8 Appendix: Approximative band structure calcuations
...
## 2.9 Appendix: Representations of a few point groups
...
## 2.10 Appendix: Nearly free electron approximation - RayleighSchrödinger formulation
...
## 3.1 The band structure of the elements in group IV

### 3.1.1 Crystal and band structure
...
Figure 2.2: The crystal structure of diamond corresponds to two face-centered cubic latices shifted by a quarter of lattice spacing along the $(1,1,1)$ direction ( 2 sublattices blue and orange).
...
## 3.2 Elementary excitations in semiconductors
...
### 3.2.1 Electron-hole excitations
...
At $T=0$ the chemical potential is exactly in the center of the band gap and then moves with increasing $T$, if the effective masses are different. Note also that for small temperatures the thermally activated carriers, electrons and holes, are well described by the classical Maxwell-Boltzmann distribution, as was used in the approximation of the integrals.
...
[^5]:    ${ }^{5}$ Ambiguity of the Wannier functions: The Wannier function is not uniquely defined, because there is a "gauge freedom" for the Bloch function which can be multiplied by a phase factor
    $$
    \begin{align*}
    \psi_{\boldsymbol{k}}(\boldsymbol{r}) \rightarrow e^{i \chi(\boldsymbol{k})} \psi_{\boldsymbol{k}}(\boldsymbol{r})
    \end{align*}
    $$
    where $\chi(\boldsymbol{k})$ is an arbitrary real function. In particular, we find different degrees of localization of $w(\boldsymbol{r}-\boldsymbol{R})$ around its center $\boldsymbol{R}$ depending on the choice of $\chi(\boldsymbol{k})$.
...
### 3.5.1 Semiconductor diodes

#### 3.5.1.1 Light emitting diode
...
## 4.1 The Jellium model of the metallic state
...
### 4.1.1 Theory of metals - Sommerfeld and Pauli
...
### 4.1.2 Stability of metals - a Hartree-Fock approach
...
## 4.2 Charge excitations and the dielectric function
...
### 4.2.1 Dielectric response and Lindhard function
...
### 4.2.2 Electron-hole excitation
...
## 4.3 Lattice vibrations - Phonons
...
### 4.3.1 Vibration of a isotropic continuous medium
...
### 4.3.3 Peierls instability in one dimension
...
### 4.3.4 Dynamics of phonons and the dielectric function
...
## 4.4 Appendix: Linear Response Theory
...
### 4.4.1 Linear Response function
...
### 4.4.2 Information in the response function
...
### 4.4.4 Fluctuation-Dissipation theorem
...
# 5 Itinerant electrons in a magnetic field
...
## 5.1 The de Haas-van Alphen effect
...
### 5.1.2 Oscillatory behavior of the magnetization
...
[^17]
### 5.2.1 Hall effect of the two-dimensional electron gas
...
We now introduce an electric field $E_{y}$ along the $x$-direction. The Hamilton operator (4.40) is then modified by an additional potential $U(\boldsymbol{r})=e E_{y} y$. This term can easily be absorbed into the harmonic potential and leads to a shift of the center of the wave function,
$$
\begin{align*}
...
[^18]
### 5.2.2 Integer Quantum Hall Effect
...
\end{align*}
$$
where we sum over all occupied states and each state labelled by $k_{x}$ (determining the corresponding center $y_{0}$ ) extends over the whole length $L_{x}$ of the Hall element. Thus, the density is given by $1 / L_{x}$. The wave vector is quantized according to the periodic boundary conditions; $k_{x}=2 \pi n_{x} / L_{x}$ with $n_{x} \in \mathbb{Z}$. The velocity $v_{x}$ is given by equation (4.44). Thus, we have
$$
\begin{align*}
...
### 5.2.3 Fractional Quantum Hall Effect
...
[^2]
# 6 Landau's Theory of Fermi Liquids
...
## 6.1 Lifetime of quasiparticles
...
## 6.2 Phenomenological Theory of Fermi Liquids
...
### 6.2.1 Specific heat - Density of states
...
### 6.2.2 Compressibility - Landau parameter $\mathbf{F}_{0}^{s}$
...
### 6.2.3 Spin susceptibility - Landau parameter $\mathbf{F}_{0}^{\text {a }}$
...
### 6.2.4 Galilei invariance - effective mass and $\mathbf{F}_{1}^{s}$
...
### 6.2.5 Stability of the Fermi liquid
...
## 6.3 Microscopic considerations
...
### 6.3.3 Fermi liquid in one dimension?
...
# 7 Transport properties of metals
...
[^11]
## 7.2 Transport equations and relaxation time
...
\end{align*}
$$
is the number of particles in the infinitesimal phase space volume $d^{3} r d^{3} k /(2 \pi)^{3}$ centered at $(\boldsymbol{k}, \boldsymbol{r})$, at time $t$. Such a description is only applicable if the temporal and spacial variations occur at long wavelengths and small frequencies, respectively, i.e., if typically $q \ll k_{F}$ and $\hbar \omega \ll \epsilon_{F}$. The total number of particles $N$ is given by
$$
\begin{align*}
...
## 7.3 Impurity scattering

### 7.3.1 Potential scattering
...
Note that $W\left(\boldsymbol{k}, \boldsymbol{k}^{\prime}\right)$ does not depend on angle, meaning that the process is described by $s$-wave scattering. The energy dependence is singular at the Fermi energy, indicating that we are not dealing with simple resonant potential scattering, but with a much more subtle many-particle effect involving the electrons very near the Fermi surface. The fact that the local spins $\boldsymbol{S}_{i}$ can flip, makes the scattering center dynamical, because the scatterer is constantly changing. The scattering process of an electron is influenced by previous scattering events, leading to the singularity at $\epsilon_{F}$. This cannot be described within the first Born approximation, but requires at least the second approximation or the full solution. ${ }^{10}$ As mentioned before, the resonant
...
## 7.5 Electron-electron scattering
...
## 7.6 Matthiessen's rule and the Ioffe-Regel limit
...
## 7.7 General transport coefficients
...
[^20]
### 7.7.1 Generalized Boltzmann equation
...
## 7.8 Anderson localization in one-dimensional systems
...
### 7.8.1 Landauer Formula for a single impurity
...
### 7.8.2 Scattering at two impurities
...
### 8.1.1 Stoner model within the mean field approximation
...
[^3]
### 8.1.3 Spin susceptibility for $T>T_{C}$
...
## 8.2 General spin susceptibility and magnetic instabilities
...
### 8.2.1 General dynamic spin susceptibility
...
### 8.2.2 Instability with finite wave vector $Q$
...
\end{align*}
$$
with $\eta=\epsilon_{F} / D$ where $-D<\epsilon_{F}<+D$. For both small and large band fillings $\left(\epsilon_{F}\right.$ close to the band edges), the tendency towards ferromagnetism dominates (cf. Figure 7.5), whereas when $\epsilon_{F}$ lies in the center of the band, the susceptibility $\chi_{0}(\boldsymbol{q})$ is not maximal at $\boldsymbol{q}=0$ anymore, and magnetic ordering with a well-defined finite $\boldsymbol{q}=\boldsymbol{Q}$ becomes more probable.

### 8.2.3 Influence of the band structure
...
In quasi-one-dimensional electron systems, a main direction of motion dominates over two other directions with weak dispersion. In this case, the nesting condition is very probable to be fulfilled, as it is schematically shown in the center panel of Figure 7.6. Chromium is a threedimensional metal, where nesting occurs between a electron-like Fermi surface around the $\Gamma$-point and a hole-like Fermi surface at the Brillouin zone boundary ( $H$-point). These Fermi surfaces originate in different bands (right panel in Figure 7.6). Chromium has a cubic body centered crystal structure, where the $H$-point at ( $\pi / a, 0,0$ ) leads to the nesting vector $\boldsymbol{Q}_{x} \|(1,0,0)$ and equivalent vectors in $y$ - and $z$-direction, which are incommensurable with the lattice.
...
# 9 Magnetism of localized moments
...
### 9.1.4 Fermi liquid properties of the metallic state
...
## 9.2 The Mott insulator as a quantum spin system
...
### 9.2.1 The effective Hamiltonian
...
### 9.2.2 Mean field approximation of the anti-ferromagnet
...
## 9.3 Collective modes - spin wave excitations
...
# 10 Appendix: Identical Quantum Particles - Formalism of Second Quantization
...
## 10.1 Many-body wave functions and particle statistics
...
## 10.2 Independent, indistinguishable particles
...
## 10.3 Second Quantization Formalism
...
### 10.3.1 Creation- and annihilation operators
...
## 10.4 Observables in second quantization
...
### 10.7.2 Bose-Einstein condensate and coherent states
...
[^13]
### 10.7.3 Phonons in an elastic medium
...
[^5]:    ${ }^{1}$ In nature, this can only be induced by enormous pressures metallic hydrogen probably exists in the centers of the large gas planets Jupiter and Saturn due to the gravitational pressure.
...
[^9]:    ${ }^{5}$ Lattices which allow for such a separation are called bipartite. There are lattices, where this is not possible, e.g., triangular or cubic face centered lattices. There, frustration phenomena appear, a further complication of anti-ferromagnetically coupled systems.
