---
{"dg-publish":true,"permalink":"/notes/2025-quantum-electronics-gallmann/8-laser-fundamentals/","hide":"true","updated":"2025-05-06T13:22:05.000+02:00"}
---
...
In the case of homogeneous broadening, the broadening of the emission of individual atoms is the same across all atoms. This is because each atom 'sees' the same environment.
...
Two examples are lifetime broadening, assuming all atoms are the same. Another is collision broadening, where each atom 'suffers the same' from elastic or inelastic collisions, mainly because there are so many atoms. On the other hand, inhomogeneous broadening considers the emission from the ensemble of many atoms. Each atom sees a different environment and thus a different resonance frequency.
...
This is for example in Doppler broadening. Inhomogeneous broadening occurs when atoms experience slightly different local environments or conditions, leading to a distribution of transition energies within the ensemble. Then, the transition energy varies slightly from atom to atom within the ensemble. While there is broadening, the actual resonance frequency of each atom has shifted, instead of just broadening around the resonance. 

Let us consider lifetime broadening again. The intensity of light emitted by an ensemble of initially excited atoms through spontaneous emission, given a lifetime $t_{sp}$, behaves as
$$
...
$$
	with $\Delta f=\frac{1}{2\pi t_{sp}}$ the lifetime-broadening linewidth. 
...
Then, considering that a Doppler broadened spectrum consists of individual, shifted Lorentzian spectra, the total spectrum is then generally a Gaussian spectrum for inhomogeneous broadening.
...
- Spectral hole burning occurs in inhomogeneously broadened gain media. Only those atoms whose resonance is near the incoming light frequency $f_0$ will interact strongly and become saturated. Once saturated, those atoms no longer contribute to gain at $f_0$. This reduces the gain at $f_0,$ making a 'hole' in the gain spectrum at that frequency.
...
| Yb-glass                     | Fiber         | 1030 nm                                             | CW-fs                         | W-kW         | Materials processing, ultrashort pulse research, LIDAR                                                      |
