...
---
## Generating Circularly Polarised High Harmonics
[[Reading/Papers/Attosecond/Generation of circularly polarised high harmonics.pdf#page=1&selection=0,0,1,35|•]]

Following the paper ["Non-collinear generation of angularly isolated circularly polarized high harmonics"](https://www.nature.com/articles/nphoton.2015.181). One way to generate circularly polarised HHG is by non-collinearly focusing a left- and right-circularly polarised femtosecond laser beam into a gas target. The advantage to other methods is that a single-colour driving laser can used, which maximises the obtained photon energies. Second, due to the collinear setup, the generated high harmonic radiation is angularly isolated. Lastly, the crossing angle between the two driving lasers allows angularly separating the radiation without need for a spectrometer:
...
The non-collinear HHG can be unterstood in two pictures:
- Photon picture: This method is governed by simple selection rules taking into account conservation of parity, linear momentum and angular momentum, see next image (a) and (c). Compared to the linearly polarised case, conservation of spin angular momentum dictates that a given harmonic can only produce two beams (c).
- Wave picture: The circularly polarised high harmonics are generated by driving that are locally linear. This enables the optical cutoff photon energy of linearly polarised HHG. Considering a non-collinear mixing, the crossing of two linearly polarised beams (b) produces and interference pattern at the focal plane. Due to the high harmonic yield being highly nonlinear with respect to the driving field, there are many diffracted orders in the far field. In contrast, the crossing of two circularly polarised, counter-rotating beams (d) does not produce an interference pattern at the crossing plane, because the polarisations are orthogonal. However, provided that the amplitudes of the beams are equal, the combined polarisation is locally linear.  The orientation of this linear polarisation varies however. It is therefore a rotating polarisation gating. 

For the mixing of linearly polarised laser beams, the only constraint is that the total number of absorbed photons is **odd**. This allows multiple combinations of photons for any given harmonic (in this case the 7th) and results in XUV beams emerging at different angles. 
...
Both beams need to be overlapped both in time and space, otherwise there is no HHG due to the circular polarisation of the individual laser beam. Two XUV beams are produced for each odd harmonic order, with one circularly polarised beam propagating to the right, and the other to the left with opposite helicity:
